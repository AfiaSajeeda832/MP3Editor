...
static int dump_header_Print(mp3header *header, FILE *out)
{
...
int dump_Print(FILE *in, FILE *out)
{
  mp3header header;
  ...
  //for (int i=0;i<frameAddress.size();i++)
    //cout<<i+1<<" "<<frameAddress.at(i)<<endl;
...
void storeByte(vector <long int> frameAddress,vector <string> audioblock)
{
...
void printaudio(vector <string> audioblock)
{
